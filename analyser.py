...
class twitterquery:
    ...
    def get_twitter_response():
        try:
            ...
            tweets = api.get_tweets(query='USDJPY', count=1000)
...
class oandaquery:
    def stream():
        ...
        bids = float(prices['prices'][0]['asks'][0]['price'])
        asks = float(prices['prices'][0]['bids'][0]['price'])
        oandatime = datetime.datetime.now()
        return bids, asks, oandatime


if __name__ == "__main__":
    while True:
        try:
            prices = oandaquery.stream()
            print(prices)
            time.sleep(2)
        except:
